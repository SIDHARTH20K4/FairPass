...
interface IEventChainContract {
     ...
     */
    function safeMint(address to, string memory uri, string memory eventDetails, uint256 originalPrice, uint256 expirationDate,uint256 eventId,
        bool resaleFeeEnabled) external returns (uint256);
    function markPending(uint256 ticketId) external;
    function approveTicket(uint256 ticketId) external;
     ...
     */
    function transferWithHistoryUpdate(address from, address to, uint256 tokenId,uint256 salePrice,
    address organizer) external;
